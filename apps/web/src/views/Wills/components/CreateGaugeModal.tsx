...
const CreateGaugeModal: React.FC<any> = ({ variant = 'user', pool, currAccount, currToken, currency, onDismiss }) => {
  ...
  const [state, setState] = useState<any>(() => ({
    ...
    willWithdrawalPeriod: parseInt(pool?.willWithdrawalPeriod ?? '0') / 60,
    uriGenerator: '',
    autoCharge: 0,
    like: 0,
    ...
    name: pool?.name,
    collectionId: '',
    updatePeriod: parseInt(pool?.updatePeriod ?? '0') / 60,
