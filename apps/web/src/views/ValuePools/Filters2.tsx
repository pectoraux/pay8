...
import { ListTraitFilter } from './ListTraitFilter2'
...
const Filters: React.FC<any> = ({ tags, nftFilters, workspace = true }) => {
  ...
  return (
    <>
      <ScrollableFlexContainer>
        ...
        <ListTraitFilter key="country" title={capitalize('country')} traitType="country" items={countryItems} />
        <ListTraitFilter key="city" title={capitalize('city')} traitType="city" items={cityItems} />
        {productItems?.filter((val) => !!val.label)?.length ? (
          <ListTraitFilter
            key="product"
